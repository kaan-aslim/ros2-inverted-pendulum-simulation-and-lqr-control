...
* gravity,
* cart and pendulum masses,
* pendulum length, radius, centre-of-mass location and mass moment of inertia,
* horizontal control force,
* coupling between cart and pendulum motion.
...
The pendulum is a uniform rigid cylinder connected to the cart through a revolute joint.
...
Because the pendulum has distributed mass, its rotational resistance is represented by its mass moment of inertia. The model therefore retains both the translational motion of the pendulum centre of mass and the rotation of the pendulum about its centre of mass.
...
For a uniform cylinder with a constant cross-section and uniform density:
...
The cylinder radius is denoted by:
...
The gravitational moment and the translational motion of the centre of mass depend on $l$, while the rotational inertia depends on the complete mass distribution defined by $m$, $L$ and $r$.
...
The rigid pendulum can be analysed using:
...
The rotational equation about the pivot has the general structure:
...
The pivot-axis inertia is obtained from the centre-of-mass inertia using the parallel-axis theorem:

$$
I_{\mathrm{pivot}}=I+ml^2
$$

Throughout this project, $I$ denotes the pendulum mass moment of inertia about its centre of mass.
...
| $I$      | Pendulum inertia about its centre of mass       | kg$\cdot$m$^2$ |
| $I_{\mathrm{pivot}}$ | Pendulum inertia about the pivot axis     | kg$\cdot$m$^2$ |
...
The pendulum inertia depends on its distributed mass, geometry and axis of rotation.

The URDF pendulum is a uniform rigid cylinder. Its mass moment of inertia about an axis through its centre of mass and perpendicular to its longitudinal axis is:
...
The dynamic derivation uses $I$ together with the parallel-axis term $ml^2$. Equivalently, the combined quantity $I+ml^2$ represents the pendulum inertia about the pivot.
...
### 11.5 Uniform Rigid-Cylinder Pendulum

The pendulum is modelled as a uniform rigid cylinder. Its centre of mass is located at:

$$
l=\frac{L}{2}
$$

Its mass moment of inertia about the centre of mass is:

$$
I=\frac{1}{12}m(3r^2+L^2)
$$

This representation preserves the pendulum's distributed mass and rotational inertia.
...
| $L$                   | Total pendulum length            | Length of `pendulum_link`                                        |
| $l$                   | Pivot-to-centre-of-mass distance | Distance from joint origin to inertial origin of `pendulum_link` |
| $r$                   | Pendulum radius                  | Radius of the cylindrical `pendulum_link`                        |
| $I$                   | Pendulum centre-of-mass inertia  | Transverse component of the `pendulum_link` inertia tensor       |
| $I_{\mathrm{pivot}}$  | Pendulum pivot-axis inertia      | $I+ml^2$ from the parallel-axis theorem                          |
| $x$                   | Cart position                    | Position of `cart_rail_joint`                                    |
| $\dot{x}$             | Cart velocity                    | Velocity of `cart_rail_joint`                                    |
| $\theta$              | Pendulum angle                   | Position of `pendulum_cart_joint`                                |
| $\dot{\theta}$        | Angular velocity                 | Velocity of `pendulum_cart_joint`                                |
...
* the pendulum inertial origin must match the assumed centre of mass,
* masses must match the values used in the equations,
* the centre-of-mass inertia must correspond to the same cylindrical geometry and rotation axis,
* the force command direction must match positive $x$,
* the reported pendulum angle must match the selected positive $\theta$ direction.
...
* a fixed horizontal rail,
* a cart of mass $M$,
* a uniform rigid-cylinder pendulum of mass $m$, length $L$ and radius $r$,
* a pendulum centre of mass located at distance $l$ from the pivot,
* a centre-of-mass inertia $I$ and pivot-axis inertia $I_{\mathrm{pivot}}=I+ml^2$,
* one prismatic coordinate $x$,
* one revolute coordinate $\theta$,
* gravity acting downward,
* a horizontal control force $F$ applied to the cart,
...
This model contains the minimum information required to derive the nonlinear rigid-body equations of motion.
...
1. write the horizontal force balance for the cart,
2. write the rigid-body rotational equation for the pendulum using $I+ml^2$,
3. eliminate the unknown pivot forces,
4. obtain the coupled nonlinear equations of motion,
5. prepare the model for linearisation and state-space representation.
